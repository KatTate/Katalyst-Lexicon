...
function TermCardContent({ term, highlightQuery, variant = "card" }: TermCardProps) {
  ...
  return (
    <div className={cn(
      ...
      isDeprecated && (isInline ? "opacity-75" : "opacity-75 bg-muted/30")
      ...
      <div className={cn("flex items-start justify-between", !isInline && "mb-4")}>
        <div className="space-y-1 min-w-0 flex-1 mr-2">
          ...
          <p className={cn("text-muted-foreground font-bold uppercase tracking-wide", isInline ? "text-[10px]" : "text-xs")}>
            {term.category}
          </p>
        </div>
      ...
      <div className={cn(
        ...
        isInline ? "pt-1" : "mt-auto pt-4 border-t border-border/40"
      )}>
...
export function TermCard({ term, highlightQuery, variant = "card" }: TermCardProps) {
  if (variant === "inline") {
    return <TermCardContent term={term} highlightQuery={highlightQuery} variant="inline" />;
  }

  return (
    <Link href={`/term/${term.id}`}>
      <TermCardContent term={term} highlightQuery={highlightQuery} variant="card" />
    </Link>
  );
}
